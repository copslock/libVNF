		...
		case 3:
			...
			if(current_context.registered == 0)
			{
				TRACE(cout<<imsi<<"has been deregistered successfully\n";)
		        	delData(conn_id, id, x->imsi, "local");
			}
			else
			{
...
void handleRegistrationRequest(int conn_id, int id, void* request, char* packet){
	...
	UEcontext current_context; // Stores current UEContext	
	mdata *x = static_cast<mdata*>(request);
	icscf_fd = createClient(id, PCSCFADDR, ICSCFADDR, ICSCFPORTNO, "tcp"); 
	memmove(&packet_length, packet, sizeof(int)); // Move packet length into packet_len
    if(packet_length <= 0)
        {
            perror("Error in reading packet_length\n");
	...
	switch(pkt.sip_hdr.msg_type) // Read packet here
	{
		case 1:
			...
			if (ENC_ON) // Add encryption
		        ...
		        pkt.prepend_sip_hdr(x->sipheader);
		        pkt.prepend_len();
		        addReqCtxt(icscf_fd, id, request);
		        registerCallback(icscf_fd, id, "read", handlecase3);
		        pkt1 = writePktmem(id);
		        memcpy((void*)pkt1, (void*)(pkt.data), pkt.len);
		        sendData(icscf_fd, id, pkt1, pkt.len);
...
void handleregreq_auth(int conn_id, int id, void* request, void* packet){
	...
	break;
        ...
        pkt.prepend_sip_hdr(x->sipheader);
        pkt.prepend_len();
        addReqCtxt(x->icscf_fd, id, request);
        registerCallback(x->icscf_fd, id, "read", handlecase3);
        char* pkt1 = writePktmem(id);
        memcpy((void*)pkt1, (void*)(pkt.data), pkt.len);
        sendData(x->icscf_fd, id, pkt1, pkt.len);
