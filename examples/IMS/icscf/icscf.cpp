...
void handlecase5(int conn_id, int id, void* request, char* packet){
	...
	switch(x->sipheader)
	{
		case 1:
			...
			getData(conn_id, id, imsi, "local", handlecase5_get);
			break;
		case 2:
...
void handlecase3(int conn_id, int id, void* request, char* packet){
	...
	int scscf_fd;
	scscf_fd = createClient(id, ICSCFADDR, SCSCFADDR, SCSCFPORTNO, "tcp"); 
	mdata *x = static_cast<mdata*>(request);
	x->scscf_fd = scscf_fd;
	memmove(&packet_length, packet, sizeof(int)); // Move packet length into packet_len
	if(packet_length <= 0)
...
void handlecase3_get(int conn_id, int id, void* request, void* packet){
	...
	pkt.prepend_sip_hdr(x->sipheader);								
	pkt.prepend_len();
	addReqCtxt(x->scscf_fd, id, request);
    registerCallback(x->scscf_fd, id, "read", handlecase5);
	char* pkt1 = writePktmem(id);
    memcpy((void*)pkt1, (void*)(pkt.data), pkt.len);
    sendData(x->scscf_fd, id, pkt1, pkt.len);
}
void handleRegistrationRequest(int conn_id, int id, void* request, char* packet){
	...
	UEcontext current_context; // Stores current UEContext	
	mdata *x = static_cast<mdata*>(request);
	hss_fd = createClient(id, ICSCFADDR, HSSADDR, HSSPORTNO, "tcp"); 
	memmove(&packet_length, packet, sizeof(int)); // Move packet length into packet_len
    if(packet_length <= 0)
        {
            perror("Error in reading packet_length\n");
