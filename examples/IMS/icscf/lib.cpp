...
struct epoll_event ev_arr[MAX_THREADS];
/*----------------------------------------------------------------------------*/
time_t start_app,end_app;
double rate=0;
double sec=0;
double packets[7]={0,0,0,0,0,0,0};
mutex mct,eparr,sock_c,f_ptr_lock,mp_lock,ds_lock,ds_conn_lock,listen_lock,evarr;
...
//std::vector<char> mp_ds(64*131072);  //assuming value size 64 TODO  //working feb22
std::vector<char> mp_ds(1024*32768);  //assuming value size 64 TODO  //working feb22
//std::vector<char> mp_ds(64*2048);  //assuming value size 64 TODO
unordered_map<uint64_t, void*> ds_map1;   //data store if option is local //TODO make it general using boost
unordered_map<void*, int>local_list; //local list of addr for clearing cache..local dnt remove
unordered_map<int, void*>cache_list; //cache list of addr for clearing cache..cache remove
unordered_map<void*, int>cache_void_list; //cache list of addr for clearing cache..cache remove
unordered_map<void*, int>reqptr_list;  //list of addr pointed in req object needed for clearing cache..pointed dnt remove
...
int ds_threshold = 131072, ds_sizing=1; //working feb22
//int ds_threshold = 2048, ds_sizing=1;
...
void populate_port_ds(char* client_ip, int client_start_port, char* server_ip1) {
	...
	if(numth==1)
        {
                p_core[0].ds_client_port = client_start_port;
        	client_start_port = client_start_port+1;
                p_core[0].ds_client_port1 = client_start_port;
                return;
        }
...
int ds_conn=1;
	...
	mp_v1.resize((p_core[id].memory_size[0])*524288);
...
//      mp_v.resize(memory_size*1000000); //uncomment nov22
	...
	if(ds_sizing==1){
	        ...
	        storageds.add_block(&mp_ds.front(), mp_ds.size(), 256);  //IMS
		ds_sizing=0;
	}
	ds_lock.unlock();
...
//						pkt_recv_ds++;
						...
						int rd = read(events[i].data.fd, data, BUFSIZE); //5 replace with BUFSIZE
						if(rd <= 0){
                                                	//will come here during connection close check?? TODO
	                                                //cout<<"Connection closed with client"<<endl;
						...
						fn_ptr(events[i].data.fd, id, p_core[id].mem_ptr[events[i].data.fd], data);
...
//void setData(int connID, int id, uint64_t key, string localRemote, string value){
void setData(int connID, int id, uint64_t key, string localRemote, void* value, int value_len){
...
//void getData(int connID, int id, uint64_t key, string localRemote, void callbackFnPtr(int, int,  void*, char*)){
void getData(int connID, int id, uint64_t key, string localRemote, void callbackFnPtr(int, int,  void*, void*)){
//	registerCallback(connID, id, "read", callbackFnPtr);
	...
	//       sendData(ds_sockid, id, pkt.data, pkt.len);
		...
		if(p_core[id].get_c==0){
		 ...
		 ds_lock.unlock();
		 //cout<<"get value "<<ds_value<<endl;
		 //cout<<"get value length "<<strlen(ds_value)<<endl;
		 //f_ptr_lock.lock(); //TODO add lock
     //            fn_ptr = p_core[id].funct_ptr[connID];
                 //f_ptr_lock.unlock();
       //          fn_ptr(connID, id, p_core[id].mem_ptr[connID], ds_value);
		callbackFnPtr(connID, id, p_core[id].mem_ptr[connID], ds_value);
                
		
	}
}

void delData(int connID, int id, uint64_t key, string localRemote){
	if(localRemote=="remote"){
...
void closeConnection(int id, int conn_id){
	close(conn_id);
}
