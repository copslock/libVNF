...
void Ran::register1() {
	...
	TRACE(cout<<"Registration process for "<<ran_ctx.imsi <<" started"<<endl;)
	pkt.prepend_sip_hdr(1);
	pcscf_client.snd(pkt);
	pkt.clear_pkt();
	pcscf_client.rcv(pkt);
	pkt.extract_sip_hdr();
	pkt.extract_item(privateidentity);
	pkt.extract_item(ran_ctx.imsi);
	pkt.extract_item(ran_ctx.gruu);
	pkt.extract_item(ran_ctx.rand_num);
	pkt.extract_item(ran_ctx.xautn_num);
	pkt.extract_item(ran_ctx.k_asme);
	pkt.extract_item(status);

	TRACE(cout<<"Message from PCSCF for "<<ran_ctx.imsi<<" is "<<status<<endl;)				
	...
	if (ran_ctx.autn_num != ran_ctx.xautn_num) {
		TRACE(cout << "register1:" << " authentication of SCSCF failure: " << ran_ctx.imsi << " "<<ran_ctx.autn_num<<" "<<ran_ctx.xautn_num<<" "<<ran_ctx.res<<" "<<ran_ctx.key<<" "<<ran_ctx.rand_num<<endl;)
		exit(1);
	}
	{
	TRACE(cout <<"SCSCF Authenticated successfuly by " << ran_ctx.imsi << endl;)
	}


	ran_ctx.ck = ran_ctx.res + 2;
	ran_ctx.ik = ran_ctx.res + 3;

	TRACE(cout<<"Authentacation details received/calculated ";)
	TRACE(cout<<ran_ctx.k_asme<<" "<<ran_ctx.res<<" "<<ran_ctx.ck<<" "<<ran_ctx.ik<<" "<<endl;)
	*/
}
