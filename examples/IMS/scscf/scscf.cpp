...
UEcontext::UEcontext(){
        ...
        expiration_time=0;
        registered=0;
        scscf_addr=""; // Stores IP Address of SCSCF
	...
	switch(x->sipheader)
	{
		case 1:// Case 1, got authentication challenge from HSS
			...
			TRACE(cout<<"Managed to get authorization stuff"<<x->autn_num<<" "<<x->rand_num<<" "<<x->xres<<" "<<x->k_asme<<endl;)
                        getData(conn_id, id, imsi, "local", handlecase3_get);
			break;
		case 2:
			...
			pkt.append_item(status);
			  if (ENC_ON) // Add encryption
		        ...
		        sendData(pcscf_fd, id, pkt1, pkt.len);
		        freeReqCtxt(pcscf_fd, id, 1);
		        closeConnection(id, pcscf_fd);
			break;
		case 3:
			freeReqCtxt(conn_id, id, 1);
		        closeConnection(id, conn_id);
...
void handlecase3_get(int conn_id, int id, void* request, void* packet){
	...
	switch(x->sipheader)
	{
		case 1:// Case 1, sending authentication challenge from SCSCF to ICSCF
		    ...
		    setData(conn_id, id, x->imsi, "local", &current_context, sizeof(UEcontext));
...
void handleRegistrationRequest(int conn_id, int id, void* request, char* packet){
	...
	UEcontext current_context; // Stores current UEContext	
	mdata *x = static_cast<mdata*>(request);
	hss_fd = createClient(id, SCSCFADDR, HSSADDR, HSSPORTNO, "tcp"); 
	memmove(&packet_length, packet, sizeof(int)); // Move packet length into packet_len
    if(packet_length <= 0)
    {
            perror("Error in reading packet_length\n");
	...
	switch(x->sipheader)
	{
		case 1:
			...
			TRACE(cout<<"ID"<<id<<" "<<current_context.instanceid<<" "<<current_context.expiration_value<<" "<<current_context.integrity_protected<<endl;)
			current_context.gruu = g_telecom.get_gruu(current_context.imsi,current_context.instanceid);
			setData(conn_id, id, imsi, "local", &current_context, sizeof(UEcontext));
            handleregreq_auth(conn_id, id, request, packet);
			break;
		case 2:
			...
			getData(conn_id, id, imsi, "local", handleregreq_auth);
			break;
		case 3:
            pkt.extract_item(x->instanceid);
            pkt.extract_item(x->expiration_value);
...
void handleregreq_auth(int conn_id, int id, void* request, void* packet){
    ...
    pkt.prepend_sip_hdr(x->sipheader);
    pkt.prepend_len();
	addReqCtxt(x->hss_fd, id, request);
	registerCallback(x->hss_fd, id, "read", handlecase3);
	char* pkt1 = writePktmem(id);
	memcpy((void*)pkt1, (void*)(pkt.data), pkt.len);
    sendData(x->hss_fd, id, pkt1, pkt.len);
}
