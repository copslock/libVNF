...
//	data = g_utils.allocate_uint8_mem(BUF_SIZE);
	data  = (uint8_t*)writePktmem(id);
	data_ptr = 0;
	len = 0;
}

Packet::Packet(const Packet &SRC_OBJ) {
	sip_hdr = SRC_OBJ.sip_hdr;
	data = g_utils.allocate_uint8_mem(BUF_SIZE);
...
void swap(Packet &src_obj, Packet &dst_obj) {
	...
	swap(src_obj.sip_hdr, dst_obj.sip_hdr);
...
void Packet::prepend_sip_hdr(int msg_type) {
	uint8_t *tem_data = g_utils.allocate_uint8_mem(BUF_SIZE);	
	int hdr_len = SIP_HDR_LEN;

	sip_hdr.init(msg_type);
	memmove(tem_data, &sip_hdr, hdr_len * sizeof(uint8_t));
...
void Packet::extract_sip_hdr() {
	int hdr_len = SIP_HDR_LEN;

	memmove(&sip_hdr, data + data_ptr, hdr_len * sizeof(uint8_t));
	data_ptr += hdr_len;
}
