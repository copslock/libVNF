...
LibPacket::LibPacket(const LibPacket &SRC_OBJ) {
...
void swap(LibPacket &src_obj, LibPacket &dst_obj) {
	using std::swap;
...
LibPacket& LibPacket::operator=(LibPacket src_obj) {
	swap(*this, src_obj);
	return *this;	
}

LibPacket::LibPacket(LibPacket &&src_obj)
	:LibPacket() {
	swap(*this, src_obj);	
}

void LibPacket::append_item(bool item) {
...
void LibPacket::append_item(int item) {
...
void LibPacket::append_item(uint8_t item) {
...
void LibPacket::append_item(uint16_t item) {
...
void LibPacket::append_item(uint32_t item) {
...
void LibPacket::append_item(uint64_t item) {
...
void LibPacket::append_item(vector<uint64_t> item) {
	int i;
...
void LibPacket::append_item(uint8_t *item, int item_len) {
	memmove(data + data_ptr, item, item_len * sizeof(uint8_t));
	data_ptr += item_len;
	len += item_len;
}

void LibPacket::append_item(const char *ITEM) {
...
void LibPacket::append_item(string item) {
...
void LibPacket::prepend_item(uint8_t *item, int item_len) {
...
/*void LibPacket::prepend_gtp_hdr(uint8_t protocol, uint8_t msg_type, uint16_t msg_len, uint32_t teid) {
	uint8_t *tem_data = g_utils.allocate_uint8_mem(BUF_SIZE);
...
void LibPacket::prepend_s1ap_hdr(uint8_t msg_type, uint16_t msg_len, uint32_t enodeb_s1ap_ue_id, uint32_t mme_s1ap_ue_id) {
	uint8_t *tem_data = g_utils.allocate_uint8_mem(BUF_SIZE);	
...
void LibPacket::prepend_diameter_hdr(uint8_t msg_type, uint16_t msg_len) {
	uint8_t *tem_data = g_utils.allocate_uint8_mem(BUF_SIZE);	
...
void LibPacket::extract_item(bool &item) {
	int item_len = sizeof(bool);
...
void LibPacket::extract_item(int &item) {
	int item_len = sizeof(int);
...
void LibPacket::extract_item(uint8_t &item) {
	int item_len = sizeof(uint8_t);
...
void LibPacket::extract_item(uint16_t &item) {
	int item_len = sizeof(uint16_t);
...
void LibPacket::extract_item(uint32_t &item) {
	int item_len = sizeof(uint32_t);
...
void LibPacket::extract_item(uint64_t &item) {
	int item_len = sizeof(uint64_t);
...
void LibPacket::extract_item(vector<uint64_t> &item, int item_size) {
	int i;
	int item_ele_len = sizeof(uint64_t);
...
void LibPacket::extract_item(uint8_t *item, int item_len) {
	memmove(item, data + data_ptr, item_len * sizeof(uint8_t));
	data_ptr += item_len;
}

void LibPacket::extract_item(char *item, int item_len) {
	memmove(item, data + data_ptr, item_len * sizeof(uint8_t));
	data_ptr += item_len;
}

void LibPacket::extract_item(string &item) {
	char *citem;
	int item_len;
...
/*void LibPacket::extract_gtp_hdr() {
	int hdr_len = GTP_HDR_LEN;
...
void LibPacket::extract_s1ap_hdr() {
	int hdr_len = S1AP_HDR_LEN;
...
void LibPacket::extract_diameter_hdr() {
	int hdr_len = DIAMETER_HDR_LEN;
...
struct ip* LibPacket::allocate_ip_hdr_mem(int len) {
	struct ip *ip_hdr;
