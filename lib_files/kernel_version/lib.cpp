        ...
        for(int i=0;i<nevents;i++) {
            if(	(events[i].events & EPOLLERR) ||
				...
				while(1){ 
                    ...
                    fn_ptr(events[i].data.fd, id, p_core[id].mem_ptr[events[i].data.fd], data);//callback function called
