...
std::vector<char> mp_ds(64*131072);  //size of memory pool data store. may have to increase this at high load. assuming value size 64  
...
int ds_portno[4] = {7000,7001,7002,7003};  //2 connections/core with data store (DS)
			...
			if(     (events[i].events & MTCP_EPOLLERR) ||
            	...
            	if(events[i].data.sockid == p_core[id].ds_sockid || events[i].data.sockid == p_core[id].ds_sockid1){
            		//reply from data store
                        ...
                        fn_ptr(buf_sockid, id, p_core[id].mem_ptr[buf_sockid], buf_value.c_str()); //callback function initialised
				        }
				}  
                else{
                		//packet from othe VNFs/ client
						data = static_cast<char*>(p_core[id].storagepkt.malloc()); //put the read data in packet pool
						int rd = mtcp_read(mctx, events[i].data.sockid, data, BUFSIZE); 
						if(rd <= 0){
                        	mtcp_epoll_ctl(mctx, ep, MTCP_EPOLL_CTL_DEL, events[i].data.sockid, NULL);
                        ...
                        fn_ptr(events[i].data.sockid, id, p_core[id].mem_ptr[events[i].data.sockid], data); //callback function called
